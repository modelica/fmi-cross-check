...
FMU importing tools must successfully import at least 3 FMUs for at least one supported platform of at least 3 exporting tools (3&times;3 FMUs per platform) by other vendors and run for as long as the `{Model_Name}_ref.opt` states and supply a CSV file of the solution they computed to receive a green status (Filtering – e.g. by platform – will lead to limiting the display of tools). FMUs generated by different versions of an exporting tool and having the same name do not count as different FMUS. FMUs marked as non compliant with the latest rules do not count neither.
